    ...
    public partial class Stock : Form
    {
        ...
        private void btncheck_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (chkAssemblyProduct.Checked == true)
                {
                    try
                    {
                        DataTable dt = new DataTable();
                ...
                else if (chkColor.Checked == true)
                {
                    try
                    {
                        DataTable dt = db.GetTable("Select Id,ProductName,MfgCompany,Unit,Size,OpeningQty,PurchaseQty,PurReturnQty,SaleQty,SaleReturnQty,DeadQty,RemQty,ProductRate,RemQty*ProductRate as Amount from Stock where Size='" + txtColor.Text + "'");
                        GvPorderInfo.DataSource = null;
                        for (int i = 0; i < GvPorderInfo.Columns.Count; i++)
                        {
                            GvPorderInfo.Columns.RemoveAt(i);
        ...
        private void chkAssemblyProduct_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (chkAssemblyProduct.Checked == true)
                {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtProductname.Text != "")
                {
                    RptRemainingtock remainingqty = new RptRemainingtock(null, txtProductname.Text, 'y');
                    remainingqty.ShowDialog();
                    remainingqty.Dispose();
                }
                else if (cmbcomanyname.Text != "Select" && cmbcomanyname.SelectedIndex > 0)
                {
                    RptRemainingtock remainingqty = new RptRemainingtock(cmbcomanyname.Text, null, 'y');
                    remainingqty.ShowDialog();
                    remainingqty.Dispose();
                }
                ...
                {
                    string d = dtpfromdate.Value.Date.ToString("MM/dd/yyyy");
                    RptRemainingtock remainingqty = new RptRemainingtock(d, 'y');
                    remainingqty.ShowDialog();
                    remainingqty.Dispose();
                }
                else
        ...
        private void chkColor_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                cmbGroupName.SelectedIndex = 0;
                chkDate.Checked = false;
                chkAssemblyProduct.Checked = false;
            }
            catch (Exception)
            { }
        }
