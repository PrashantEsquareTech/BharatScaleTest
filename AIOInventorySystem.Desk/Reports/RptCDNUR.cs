...
{
    public partial class RptCDNUR : Form
    {
            ...
            try
            {
                ...
                if (dtdetail.Rows.Count > 0)
                {
                    for (int i = 0; i < dtdetail.Rows.Count; i++)
                    {
                        ...
                        DataTable dtcust = db.Gettable("select cust.state,cust.statecode as cstatecode,comp.statecode as costatesode,cust.buyertinno from customerinformation as cust join companyinformation as comp on cust.compid=comp.companyid where comp.companyid='" + CommonMethod.CompId + "' and cust.buyertinno='' and cust.customerid=" + billcustomerid);
                        if (dtcust.Rows.Count > 0)
                        {
                            ...
                            if (dtdetail.Rows[i]["Type"].ToString() == "BillReturn")
                                dtBill = db.Gettable("Select cm.GoodReturnNo as No,cm.GoodReturnDate as Date,cd.Amount as Amt,case when (cd.SGstPercent is null or cd.SGstPercent=0) then cd.IGstPercent else (cd.SGstPercent+cd.CGstPercent) end as gst,case when cd.CessAmt is null then 0 else cd.CessAmt end as cessAmt,cd.TotalAmount,cd.Reason,cd.Rate,cd.Quantity from GoodReturnMaster as cm join GoodReturnDetail as cd on cm.Id=cd.GoodReturnNo where cm.BillNo='" + dtdetail.Rows[i]["BillId"].ToString() + "'");
                            else
                            ...
                            if (dtBill.Rows.Count > 0)
                            {
                                ...
                                datacdnur = ds.Tables["CDNURTransaction"];
                                datacdnur.Rows.Add(new object[13] { gstinno, VoucherNo, Voucherdate, DType, billno, billdate, Reason, Placeofsupply, VoucherValue, gstrate, taxableamt, cessamt, PreGst });
                            }
                        }
                    }
                }
                ...
                CDNURSummaryReport salerpt = new CDNURSummaryReport();
