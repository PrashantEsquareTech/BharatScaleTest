...
{
    public partial class RptB2B : Form
    {
        DbClass db = new DbClass();
        CommonMethod cm = new CommonMethod();
        static string strFlag = "";

        public RptB2B(string flag)
        {
            InitializeComponent();
        ...
        public void getall(string fdate, string tdate)
        {
            ...
            try
            {
                    ...
                    for (int i = 0; i < dtdetail.Rows.Count; i++)
                    {
                        ...
                        if (dtcust.Rows.Count > 0)
                        {
                            ...
                            if (hastaxes == true)
                            {
                                ...
                                errormsg = "Error on getting sale details having bill no " + dtdetail.Rows[i][0].ToString();
                                if (previousbill == "" || previousbill != billno)
                                {
                                    previousbill = billno;
                                    TotalNetAmt += netamt;
                                }
                                datab2b = ds.Tables["B2BTransaction"];
                                datab2b.Rows.Add(new object[15] { gstinno, billno, billdate, netamt, Placeofsupply, 'N', "Regular", "", gstrate, taxableamt, cessamt, "", IGSTAmt, CGSTAmt, SGSTAmt });
                            }
                        }
                        hastaxes = true;
                    }
                ...
                B2BSummaryReportNew salerpt = new B2BSummaryReportNew();
        ...
        public void getallBill(string fdate, string tdate)
        {
            try
            {
                ...
                if (strFlag == "R1")
                {
                    if (setData.ShowUserDefinedSeries == "Y")
                        da = new SqlDataAdapter("Select ci.BuyerTinNo as Gstin, cm.UserSeries as InvoiceNo,cm.billdate as InvoiceDate,cm.netamount as InvoiceValue,cast(ci.Statecode as varchar(5))+'-'+ci.State as Placeofsupply," +
                                                ...
                                                " inner join CustomerInformation as ci on cm.CustomerId=ci.CustomerID Where cm.billdate>='" + fdate + "' and cm.billdate<='" + tdate + "' " +
                ...
                {
                    da = new SqlDataAdapter("Select si.VatNo as Gstin, pm.PorderNo as InvoiceNo,pm.PorderDate as InvoiceDate,pm.NetAmount as InvoiceValue, " +
                                            "cast(sm.Id as varchar(5))+'-'+sm.StateName as Placeofsupply, " +
                                            "'N' as ReverseCharges,'Regular' as InvoiceType,'' as Ecommgstin, " +
                                            "case when (pd.sgstpercent is null or pd.sgstpercent=0) then pd.igstpercent " +
                                            "else (pd.sgstpercent+pd.cgstpercent) end as GstRate, " +
                                            "case when (pd.sgstpercent is null or pd.sgstpercent=0) " +
                                            "then Isnull(sum(Isnull(pd.Totalamount,0)),0) - Isnull(sum(Isnull(pd.IGstAmt,0)),0) " +
                                            "else Isnull(sum(Isnull(pd.Totalamount,0)),0) - Isnull(sum(Isnull(pd.CGstAmt+pd.SGstAmt,0)),0) end as TaxableValue, " +
                                            "Isnull(sum(Isnull(pd.cessamt,0)),0) as CessAmt ,'',Isnull(sum(Isnull(pd.IGstAmt,0)),0) as IGST, " +
                                            "Isnull(sum(Isnull(pd.CGstAmt,0)),0) as CGST,Isnull(sum(Isnull(pd.SGstAmt,0)),0) as SGST " +
                                            "from PurchaseMaster pm inner join PurchaseDetail pd on pm.Id=pd.POrderNo " +
                                            "inner join SupplierInformation si on si.SupplierCode=pm.SupplierId " +
                                            "inner join StateMaster sm on sm.Id=si.StateId " +
                                            "inner join Settings s on s.CompId=pm.CompId " +
                                            "where pm.PorderDate>='" + fdate + "' and pm.PorderDate<='" + tdate + "' and si.VatNo!='' " +
                                            "and pm.TransactionYear='" + CommonMethod.TransactionYear + "' and pm.CompId=" + CommonMethod.CompId + " and " +
                                            "si.CompId=(case when (s.CommonProduct='Y') then " + CommonMethod.CompId + " else si.CompId end) " +
                                            "group by si.VatNo,pm.PorderNo,pm.PorderDate,pm.NetAmount,pd.sgstpercent,pd.cgstpercent,pd.igstpercent,sm.Id,sm.StateName " +
                                            "order by pm.PorderDate ", db.Connection);
                }
                else if (strFlag == "B2CR2")
                {
                    da = new SqlDataAdapter("Select si.SupplierName as Gstin, pm.PorderNo as InvoiceNo,pm.PorderDate as InvoiceDate,pm.NetAmount as InvoiceValue, " +
                                            "cast(sm.Id as varchar(5))+'-'+sm.StateName as Placeofsupply, " +
                                            "'N' as ReverseCharges,'Regular' as InvoiceType,'' as Ecommgstin, " +
                                            "case when (pd.sgstpercent is null or pd.sgstpercent=0) then pd.igstpercent " +
                                            "else (pd.sgstpercent+pd.cgstpercent) end as GstRate, " +
                                            ...
                                            "and pm.TransactionYear='" + CommonMethod.TransactionYear + "' and pm.CompId=" + CommonMethod.CompId + " and " +
                                            "si.CompId=(case when (s.CommonProduct='Y') then " + CommonMethod.CompId + " else si.CompId end) " +
                                            "group by si.SupplierName,pm.PorderNo,pm.PorderDate,pm.NetAmount,pd.sgstpercent,pd.cgstpercent,pd.igstpercent,sm.Id,sm.StateName " +
                                            "order by pm.PorderDate ", db.Connection);
                }
                da.MissingSchemaAction = MissingSchemaAction.AddWithKey;
                ...
                {
                    B2BR2Report salerpt = new B2BR2Report();
                    salerpt.SetDataSource(ds);
                    salerpt.SetParameterValue("fromdate", dtpMonth.Value);
                    salerpt.SetParameterValue("Todate", dtptomonth.Value);
                    salerpt.SetParameterValue("TotalInvoiceValue", "0");
                    salerpt.SetParameterValue("Flag", "B2B");
                    crystalReportViewer1.ReportSource = salerpt;
                    crystalReportViewer1.Refresh();
                }
                else if (strFlag == "B2CR2")
                {
                    ...
                    salerpt.SetParameterValue("Todate", dtptomonth.Value);
                    salerpt.SetParameterValue("TotalInvoiceValue", "0");
                    salerpt.SetParameterValue("Flag", "B2C");
                    crystalReportViewer1.ReportSource = salerpt;
                    crystalReportViewer1.Refresh();
                }
                db.CloseConnection();
